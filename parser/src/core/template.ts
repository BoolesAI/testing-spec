...
export function loadTemplateChain(templatePath: string, baseDir: string, visited: Set<string> = new Set()): TSpec {
  ...
  if (template.extends) {
    ...
    return deepMerge(parentTemplate as Record<string, unknown>, templateWithoutExtends as Record<string, unknown>) as TSpec;
  }
...
export function applyTemplateInheritance(spec: TSpec, baseDir: string): TSpec {
  ...
  return deepMerge(template as Record<string, unknown>, specWithoutExtends as Record<string, unknown>) as TSpec;
}
